...
function load(){
  try{
    ...
    // Migración v10: nuevos integrantes del servicio (Haecker, Segura, Marquez).
    // Genérica: agrega cualquier miembro del listado oficial (config) que
    // falte en la instalación, sin tocar a los existentes.
    if((merged.seedVersion||1) < 10){
      const have = new Set((merged.staff||[]).map(s=>s.id));
      (def.staff||[]).forEach(d=>{
        if(!have.has(d.id)) merged.staff.push(JSON.parse(JSON.stringify(d)));
      });
...
function agendOpenDetalle(reqId){
  ...
  if(AGEND_STATE.mode === 'admin' && req.estado === 'pendiente'){
    const recibidoBtn = !req.recibido
      ? `<button type="button" class="agend-btn-secondary" onclick="agendMarcarRecibido('${req.id}',true)" style="background:#e0f2fe;border:1.5px solid #7dd3fc;color:#075985;font-weight:700">📩 Marcar recibido</button>`
      : `<button type="button" class="agend-btn-secondary" onclick="agendMarcarRecibido('${req.id}',false)" style="background:var(--tint1);border:1.5px solid var(--border);color:var(--muted)">📩 Recibido ✓ · quitar</button>`;
    actionsHtml = `
      <div class="agend-detalle-actions" style="margin-bottom:8px">${recibidoBtn}</div>
      <div class="agend-detalle-actions">
  ...
  body.innerHTML = `
    <div class="agend-detalle-card">
      <div class="agend-detalle-row"><div class="agend-detalle-k">Estado</div><div class="agend-detalle-v"><span class="agend-slot-status ${req.estado}">${req.estado}</span>${extraBadge}${(req.recibido && req.estado==='pendiente')?' <span class="agend-recibido-pill">📩 Recibido · en gestión</span>':''}</div></div>
      ${(req.recibido && req.estado==='pendiente') ? `<div class="agend-detalle-row"><div class="agend-detalle-k">Recibido</div><div class="agend-detalle-v">📩 <b>Recibido</b> — lo estamos gestionando${req.recibidoBy?` · ${_gpEsc(req.recibidoBy)}`:''}${req.recibidoAt?` · ${new Date(req.recibidoAt).toLocaleDateString('es-CL',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}`:''}</div></div>` : ''}
      ${propuestaRow}
      ${horarioRow}
...
// "Recibido": acuse de recibo (marca, NO cambia el estado). Sirve para que el
// solicitante vea que la solicitud ya llegó y se está gestionando, antes del
// visado (aprobar/proponer/rechazar). Solo modo Admin.
async function agendMarcarRecibido(reqId, on){
  const found = _agendFindRequest(reqId);
  if(!found) return;
  const data = agendLoadData();
  if(!Array.isArray(data[found.salaId][found.dateStr])){
  ...
  const slot = arr[idx];
  if(on === false){
    delete slot.recibido; delete slot.recibidoAt; delete slot.recibidoBy;
  } else {
    slot.recibido = true;
    slot.recibidoAt = Date.now();
    slot.recibidoBy = AGEND_STATE.staffNombre || 'Anestesia';
  }
  slot.updatedAt = Date.now();
  agendSaveData(data);
  if(getBackendURL()){ try{ await agendSyncNow(); }catch(e){} }
  agendOpenDetalle(reqId);
}
...
function _agendRenderSeguimiento(){
  ...
  cont.innerHTML = list.map(r => {
    ...
    // Pendiente ya acusado por el equipo → "📩 En gestión" (más que solo "Recibida").
    const ack = (r.estado === 'pendiente' && r.recibido);
    const segIco = ack ? '📩' : m.ico;
    const segLabel = ack ? 'En gestión' : m.label;
    return `
      <div class="agend-slot ${r.estado}" style="cursor:default">
        ...
        <div class="agend-seg-chip ${r.estado}" style="flex-shrink:0;align-self:center">${segIco} ${segLabel}</div>
      </div>`;
  }).join('');
}
  ...
  cont.innerHTML = list.map(r => {
    ...
    return `
      <button type="button" class="agend-slot ${r.estado}" onclick="agendOpenDetalle('${r.id}')">
        ...
        <div class="agend-slot-body">
          <div class="agend-slot-name">${_gpEsc(r.paciente)} · ${_gpEsc(r.procedimiento)}${(r.recibido && r.estado==='pendiente')?' <span class="agend-recibido-pill">📩</span>':''}</div>
